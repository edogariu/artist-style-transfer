...
# def get_avg_dataset(rescale_height=-1, rescale_width=-1):
...
#     dataset = {artist: n for artist in names}
#
#     count = 0
#     count_failed = 0
...
def get_content_dataset(size, rescale_height, rescale_width):
    ...
    for file in content_files:
        ...
        # Scaling parameters
        w_s = rescale_width / src_width
        h_s = rescale_height / src_height

        # Affine matrices for width and height
        Affine_Mat_w = [w_s, 0, rescale_width / 2 - w_s * src_width / 2]
        Affine_Mat_h = [0, h_s, rescale_height / 2 - h_s * src_height / 2]
        M = np.c_[Affine_Mat_w, Affine_Mat_h].T

        im = cv2.warpAffine(im, M, (rescale_width, rescale_height)).astype(float)
        images[count, :, :, :] = im
