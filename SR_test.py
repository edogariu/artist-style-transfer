...
IM_PATH = 'images/archive/images/images/Vincent_van_Gogh/Vincent_van_Gogh_556.jpg'
...
output = (output * 255.0).round().astype(np.uint8)  # float32 to uint8
output_swin = output
# output_swin = np.random.randn(start_size * upscale, start_size * upscale, 3)
...
# MODEL_ARGS = {'upscale_resolution': 256, 'attention_resolutions': (8, 16, 32), 'channel_mult': (1, 1, 2, 2, 4, 4),
#               'num_heads': 4, 'in_channels': 3, 'out_channels': 6, 'model_channels': 192,
#               'num_res_blocks': 2, 'resblock_updown': True, 'use_adaptive_gn': True, 'num_classes': 1000}
# model = SuperResolutionModel(**MODEL_ARGS)
# model.load_state_dict(convert_state_dict(torch.load('models/64_256_upsampler.pt', map_location="cpu")), strict=True)
# model.eval()
...
# sample = diff.denoise(x=torch.randn(size=(1, 3, 256, 256)), kwargs={'y': torch.full(size=(1,), fill_value=445),
#                                                                     'low_res': im})
