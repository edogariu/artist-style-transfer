...
class Trainer:
    def __init__(self, model, diffusion_args, dataloader,
                 ...
                 checkpoint=(None, None, None, None), test_every=None, save_every=None, device=None):
        ...
        diffusion_args.update({'rescaled_num_steps': 25, 'use_ddim': True})
    ...
    def train(self):
        for step in range(self.iterations):
            ...
            labels = torch.randint(low=0, high=1000, size=(self.batch_size,), device=self.device)
            kwargs = {'y': labels} if labels is not None else None
    ...
    def sample(self):
        self.model.eval()
        out = self.sampling_diffusion.denoise(kwargs={'y': torch.randint(low=0, high=1000, size=(self.batch_size,),
                                              device=self.device)},
...
if __name__ == '__main__':
    ...
    model.load_state_dict(convert_state_dict(torch.load('models/64x64_diffusion.pt', map_location="cpu")), strict=True)
    ...
    trainer = Trainer(model=model, diffusion_args=DIFFUSION_ARGS, dataloader=dataloader, ema_rate=0.9999,
                      iterations=100, batch_size=1, lr=0.0001, weight_decay=0.001, test_every=None,
